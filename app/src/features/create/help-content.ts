...
export const CREATE_HELP: Record<string, FieldHelp> = {
  ...
  managerFee: { brief: "Annual streaming fee paid to the manager, in bps (max 200 = 2%/yr).", extended: "Accrued continuously from NAV. Separate from Meridian's own platform AUM fee (≤0.5%/yr) and the flat USDG create/deploy fees on managed & rebalance vaults. The mint/redeem flow fee is always 0%.", example: "50 bps on a $100k index ≈ $500/yr" },
  ...
  "kind.registry": { brief: "A large registry-native index (up to 500 names) with cash create/redeem.", extended: "Like a Rebalanced vault — target weights and a keeper — but built for very large registries. Create and redeem settle for cash through a forward queue (priced at the next open), and a flat USDG create fee applies. In-kind / AP claim flows come later.", example: "An SP500-style 500-name index, cash in/out" },
};
