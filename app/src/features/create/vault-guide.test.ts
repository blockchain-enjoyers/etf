...
describe("create help content", () => {
  it("has help for every keyed field", () => {
    for (const k of ["name", "symbol", "token", "qtyPerUnit", "targetPct", "valuePerUnit", "creationUnit", "managerFee", "keeperCut", "keeperEscrow", "kind.basket", "kind.managed", "kind.committed", "kind.rebalance"]) {
      expect(CREATE_HELP[k]?.brief, k).toBeTruthy();
    }
  });
  it("comparison table covers all four kinds per row", () => {
    expect(COMPARISON.length).toBeGreaterThan(0);
    for (const r of COMPARISON) {
      expect(r.basket && r.managed && r.committed && r.rebalance).toBeTruthy();
    }
  });
  it("every guide question maps options to real kinds", () => {
    const kinds = new Set(["basket", "managed", "committed", "rebalance"]);
