...
/**
 * Registers the keeper pg-boss jobs (rebalance, settle, forward-record, forward-settle) and their
 * services. The on-chain write surfaces are accessed through capability writer ports. At L1 the
  ...
  providers: [
    ...
    { provide: SettleWriterPort, useClass: NullSettleWriter },
    {
      ...
      useFactory: (
        ...
        forwardQueues: ForwardQueueRegistry,
        queueReader: ForwardCashQueueReader,
        ...
        registry.present("BasketNavObserver") && config.get("FORWARD_OPERATOR_PRIVATE_KEY")
          ? new LiveForwardRecordWriter(chain, forwardQueues, queueReader, rebVault, signer)
    ...
    },
    {
      ...
      useFactory: (
        ...
        registry.present("ForwardCashQueue") && config.get("FORWARD_OPERATOR_PRIVATE_KEY")
          ? new LiveForwardSettleWriter(chain, forwardQueues, rebVault, signer)
