...
/// @title FeeCore — reusable streaming management + platform fee machinery (Meridian's own AUM line)
/// @notice Extracted verbatim from ManagedVault so the fee model can be reused WITHOUT
///         StorageVaultBase's on-chain recipe storage (the 500-name registry vault must NOT store a
///         500-element `_tokens` array). A streaming management fee charged by dilution: the manager sets
///         its own fee; Meridian charges its OWN independent annual platform fee (its own line, not a share
///         of the manager fee). The investor pays both legs by dilution. Fee accrues into high-precision
///         owed-accumulators; only whole shares are minted and the fractional remainder is carried (no dust
///         loss, platform cannot be starved). `is VaultCore` (the seams `_accrue`/`_chargeFlatCreateFee`
///         resolve to the overrides here); references NOTHING from StorageVaultBase.
abstract contract FeeCore is VaultCore {
    ...
    /// @dev Managed-state init, extracted so the rebalance flavors can reuse it then add keeper state.
    ...
    function _chargeFlatCreateFee() internal virtual override {
