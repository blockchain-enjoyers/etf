...
library RateLimit {
    ...
    function refill(uint256 timestamp, Storage storage limitStorage) private view returns (uint256) {
        ...
        (bool safeMul, uint256 newTokens) = Math.tryMul(secondsElapsed, limitStorage.limitConfig.refillPerSecond);
        (bool safeAdd, uint256 amount) = Math.tryAdd(limitStorage.remainingAmount, newTokens);
        if (!safeMul || !safeAdd) {
