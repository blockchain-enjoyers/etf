...
**CloneFactory (один раз, издатель):** каждый vault — это **EIP-1167 клон** фиксированной immutable-реализации (не новый байткод через CREATE2 → нет 24KB-стены фабрики). `unitSize` и `recipeCommitment` зашиты в **clone immutable-args** (читаются `Clones.fetchCloneArgs`, без SSTORE), инициализация — атомарно в той же tx через `initialize`. Три типа за одной фабрикой: **static (storage)**, **committed (off-chain рецепт)**, **managed** (см. [L1m](L1m-managed-vault.md)). cash = обычный токен в `tokens[]` (отдельной cash-ноги нет).
```
// static (рецепт хранится on-chain):
createBasket(tokens[], unitQty[], unitSize, name, symbol, userSalt) → vault   // клон BasketVault
predictBasketAddress(issuer, tokens[], unitQty[], unitSize, name, symbol, userSalt) → address

// committed (рецепт off-chain, в clone хранится только commitment — дёшево при любом N):
createCommittedBasket(tokens[], unitQty[], unitSize, name, symbol, userSalt) → vault  // клон CommittedVault
predictCommittedVaultAddress(issuer, ...) → address
...
**BasketVault — static, рецепт on-chain (вход/выход для всех):**
```
create(uint nUnits)        // ВХОД: transferFrom рецепта → _mint
createWithPermit(nUnits, PermitInput[])        // одна tx: EIP-2612 permit'ы + create
...
recipeCommitment() → bytes32                  // keccak(tokens,unitQty,unitSize) — ШОВ к L2/L4
unitSize() → uint                             // токенов корзины на 1 creation-unit
```

**CommittedVault — committed, рецепт off-chain (тот же in-kind, но рецепт в calldata):**
```
create(uint nUnits, address[] tokens, uint256[] unitQty)   // рецепт в calldata, валидируется против commitment
redeem(uint amount, address[] tokens, uint256[] unitQty)   // то же
recipeCommitment() → bytes32                                // тот же шов
// НЕТ getConstituents()/previewCreate/Redeem — рецепт on-chain не хранится.
// При деплое излучается событие RecipeCommitted(tokens, unitQty, unitSize) →
// бекенд ОБЯЗАН восстанавливать рецепт из логов (DA-tradeoff: рецепт всегда reconstructable,
// даже если бекенд оператора исчез; но caller обязан подать его в calldata на каждый create/redeem).
```
Зачем committed: дёшево деплоить корзину **на любом N** (хранится только 32-байтный commitment, нет SSTORE на каждый constituent). Платёж за это — рецепт надо носить в calldata и восстанавливать из логов.
...
- **Indexer:** слушает `Created` / `Redeemed` / `BasketCreated` / `CommittedBasketCreated`, строит ленту, supply, состав.
- **Read-API:** обёртка над view (`previewCreate/Redeem`, `getConstituents`, holdings, `recipeCommitment`).
- **Committed-волты — ОБЯЗАТЕЛЬНО:** индексировать событие `RecipeCommitted(tokens, unitQty, unitSize)` при деплое и **хранить восстановленный рецепт** — у committed-волта нет `getConstituents()`, и фронту нужно подать рецепт в calldata на каждый `create`/`redeem`. Без этого индекса корзина «нечитаема» для UI (хотя on-chain рецепт всё равно восстановим из логов).
- **Tx-builder (опц.):** собирает «approve × M + create» одним пакетом; для committed — добавляет рецепт в calldata.
- **Чего НЕТ:** оракула, beta-fit, NAV, keeper-ботов.

### 4.3 Фронтенд

- **Create (static):** ввод `N` → `previewCreate` → кнопки approve на каждый токен → `create(N)`.
- **Create (committed):** рецепт берём из BE-индекса (`RecipeCommitted`) → approve → `create(N, tokens, unitQty)` (рецепт в calldata).
- **Redeem (static):** ввод `amount` → `previewRedeem` → `redeem(amount)`.
- **Redeem (committed):** рецепт из BE → `redeem(amount, tokens, unitQty)` (preview считаем на фронте/BE из `balanceOf` vault'а, т.к. on-chain preview у committed нет).
