...
const EXPOSE = [
  ...
  { name: "MockSource", sol: "contracts/L4/mocks/MockSource.sol" },
  ...
  { name: "ForwardCashQueue", sol: "contracts/L5/ForwardCashQueue.sol", bytecode: true },
...
function loadAbis() {
  const out = [];
  for (const { name, sol, bytecode } of EXPOSE) {
    ...
    const entry = { name, abi: artifact.abi };
    if (bytecode) {
      // Backend deploys this contract via walletClient.deployContract, which needs creation bytecode.
      if (typeof artifact.bytecode !== "string" || !/^0x[0-9a-fA-F]*$/.test(artifact.bytecode)) {
        fail(`artifact ${path} has no .bytecode string (needed for deployment)`);
      }
      entry.bytecode = artifact.bytecode;
    }
    out.push(entry);
  }
  return out;
...
function renderAbis(abis) {
  ...
  for (const { name, abi, bytecode } of abis) {
    body += `export const ${name}Abi = ${JSON.stringify(abi)} as const;\n\n`;
    if (bytecode) {
      body += `export const ${name}Bytecode = ${JSON.stringify(bytecode)} as const;\n\n`;
    }
  }
  return body;
}
